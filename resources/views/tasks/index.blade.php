<x-app-layout>
                ...
                <div class="kt-card-body">
                    ...
                    <div id="month-view-container">
                        ...
                        <div id="calendar-grid" class="grid grid-cols-8 gap-1">
                            <!-- Calendar with week numbers will be generated by JavaScript -->
                        </div>
                    </div>
                        ...
                        <table class="table-kt">
                            <thead>
                                <tr>
                                    ...
                                    <th>Zespół</th>
                                    <th>
                                        <a href="{{ request()->fullUrlWithQuery(['sort' => 'start_date', 'direction' => request('sort') == 'start_date' && request('direction', 'asc') == 'asc' ? 'desc' : 'asc']) }}" 
                                           class="flex items-center space-x-1 hover:text-blue-600 dark:hover:text-blue-400">
                                            <span>Start</span>
    ...
    <script>
        ...
        function formatWeek(date) {
            ...
            return `Tydzień ${weekNum}: ${startOfWeek.getDate()} ${months[startOfWeek.getMonth()]} - ${endOfWeek.getDate()} ${months[endOfWeek.getMonth()]} ${endOfWeek.getFullYear()}`;
        }
        ...
        function renderMonthView() {
            ...
            for (let row = 0; row < rows; row++) {
                ...
                weekCell.className = 'h-40 flex items-center justify-center text-xs font-medium text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700';
                ...
                for (let col = 0; col < 7; col++) {
                    ...
                    if (dayIndex < startingDay) {
                        ...
                        cell.className = 'h-40 border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50';
                        
                        // Add previous month day number and click handler
                        const prevMonthDay = new Date(year, month, 0).getDate() - startingDay + dayIndex + 1;
                        const dayNumber = document.createElement('div');
                        dayNumber.className = 'text-xs font-medium text-gray-400 dark:text-gray-500 mb-1 cursor-pointer hover:text-gray-600 dark:hover:text-gray-300 p-1';
                        ...
                        // Add next month day number and click handler
                        const nextMonthDay = dayIndex - startingDay - daysInMonth + 1;
                        const dayNumber = document.createElement('div');
                        dayNumber.className = 'text-xs font-medium text-gray-400 dark:text-gray-500 mb-1 cursor-pointer hover:text-gray-600 dark:hover:text-gray-300 p-1';
                        dayNumber.textContent = nextMonthDay;
                        cell.appendChild(dayNumber);
                        
                        // Click handler for next month day
                        cell.addEventListener('click', () => {
